...
#define ACER_WMI_SET_FUNCTION 1
#define ACER_WMI_GET_FUNCTION 2
...
/* APGE Action */
static acpi_status wmi_apgeaction_execute(const u32 method_id, u64 input, u64 *output) {
  struct acpi_buffer in = { sizeof(u64), NULL};
  struct acpi_buffer out = { ACPI_ALLOCATE_BUFFER, NULL };

  u64 temp = 0;
  in.pointer = &input;

  const acpi_status status = wmi_evaluate_method(WMI_GUID3, 0, method_id, &in, &out);
  if (ACPI_FAILURE(status)) {
    pr_err("Failed to evaluate WMI method\n");
    return status;
  }

  const union acpi_object *obj = out.pointer;
  if (obj) {
    switch (obj->type) {
      case ACPI_TYPE_INTEGER:
        temp = (u64) obj->integer.value;
        break;
      case ACPI_TYPE_BUFFER:
        if (obj->buffer.length == sizeof(u64))
          temp = *((u64 *) obj->buffer.pointer);
        break;
      default:
        pr_err("Invalid WMI object type\n");
        kfree(out.pointer);
        return -EIO;
    }
  }
...
static int supply_get_property(struct power_supply *psy, enum power_supply_property psp, union power_supply_propval *val) {
  ...
  switch (psp) {
    case POWER_SUPPLY_PROP_CHARGE_TYPE:
      status = wmi_apgeaction_execute(ACER_WMI_GET_FUNCTION, 0x04, &wmi_result);
      if (ACPI_FAILURE(status)) {
        pr_err("Failed to get USB power-off charge status\n");
        return -ENODEV;
      }
      val->intval = wmi_result ==  663296 ? -1 :
                    wmi_result ==  659200 ? 10 :
                    wmi_result == 1314560 ? 20 :
                    wmi_result == 1969920 ? 30 :
...
static int supply_set_property(struct power_supply *psy, enum power_supply_property psp, const union power_supply_propval *val) {
  ...
  switch (psp) {
    case POWER_SUPPLY_PROP_CHARGE_TYPE:
      ...
      status = wmi_apgeaction_execute(ACER_WMI_SET_FUNCTION, value, &wmi_result);
      if (ACPI_FAILURE(status)) {
        pr_err("Failed to set USB power-off charge status\n");
        return -EIO;
      }
      break;
    case POWER_SUPPLY_PROP_CHARGE_CONTROL_END_THRESHOLD:
...
static int power_supply_init(struct platform_device *device) {
  ...
  struct power_data *data = devm_kzalloc(&device->dev, sizeof(*data), GFP_KERNEL);
  if (!data)
    return -ENOMEM;
  ...
  pr_info("Power supply module registered\n");

  return 0;
}

static void power_supply_destroy(struct platform_device *device) {
  pr_info("Power supply module removed\n");
}
...
static int __init wmi_init(void) {
  if (!wmi_has_guid(WMI_GUID1) && !wmi_has_guid(WMI_GUID2) && !wmi_has_guid(WMI_GUID3)) {
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Sametcan <scandnk52@gmail.com>");

MODULE_ALIAS("wmi:" WMI_GUID1);
MODULE_ALIAS("wmi:" WMI_GUID2);
MODULE_ALIAS("wmi:" WMI_GUID3);
